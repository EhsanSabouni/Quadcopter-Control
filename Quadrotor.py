...
class Quad:
    ...
    def motion1(self, state, input, t):
    ...
    def motion2(self, state, input, t):
        x0 = np.append(state, input)
        x = odeint(self.dynamics, x0, t)
    ...
    def rk4(self, t, state, input, n):
        ...
        t0 = t[0]
        for i in range(n):
            k1 = h * (self.dynamics(t0, x0))
            k2 = h * (self.dynamics((t0 + h / 2), (x0 + k1 / 2)))
            k3 = h * (self.dynamics((t0 + h / 2), (x0 + k2 / 2)))
            k4 = h * (self.dynamics((t0 + h), (x0 + k3)))
            k = (k1 + 2 * k2 + 2 * k3 + k4) / 6
            xn = x0 + k
            x0 = xn
            t0 = t0 + h
